...
### 1.2. Игровые сессии (`/v1/sessions`)
...
#### `POST /v1/sessions/{sessionId}/buildings`
...
### 2.2. ASCII Представление карты

### GET /maps/{id}/ascii

**Назначение:** Получение карты в текстовом ASCII формате для отладки и визуализации.

**Параметры URL:**
- `id` (string, UUID) - Идентификатор карты

**Responses:**

*   **`200 OK` - Успешное получение ASCII карты**
    ```
...
**Легенда ASCII символов:**

| Символ | Тип поверхности | Описание |
|--------|----------------|----------|
| `.` | `Dirt` | Земля |
| `#` | `Rock` | Камень |
| `X` | `Bedrock` | Неразрушимая порода |
| ` ` (пробел) | `Empty` | Пустая ячейка |
| `G` | `GoldCluster` | Скопление золота |
| `C` | `CrystalCluster` | Кристаллическое скопление |
| `I` | `IronCluster` | Железное скопление |
| `?` | Неизвестный | Неопознанный тип |

**Использование:**
- Отладка генерации карт
- Быстрая визуализация структуры карты
- Тестирование алгоритмов размещения ресурсов
...
- `terrain_dug`: Был раскопан участок карты.
  - **Payload:** `{ position: {x, y}, previousTerrain: "Rock", newTerrain: "Empty" }`
